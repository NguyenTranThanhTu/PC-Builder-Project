...
export default function ProductsManager() {
  ...
  const deleteProduct = async (p: ProductItem) => {
    if (!confirm(`⚠️ XÓA VĨNH VIỄN sản phẩm '${p.name}'?\n\nHành động này KHÔNG THỂ HOÀN TÁC!`)) return;
    
    // Double confirmation for safety
    const confirmText = prompt(`Nhập tên sản phẩm "${p.name}" để xác nhận xóa:`);
    if (confirmText !== p.name) {
      alert("Tên không khớp. Đã hủy xóa.");
      return;
    }
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center justify-center gap-2">
                          ...
                          <button 
                            ...
                            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-red text-white text-xs font-medium hover:bg-red-dark transition-colors"
