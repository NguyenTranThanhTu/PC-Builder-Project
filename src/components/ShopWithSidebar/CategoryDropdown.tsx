...
type CategoryMeta = { name: string; slug: string; productCount: number };

const CategoryItem = ({ category, selected, onToggle }: { category: CategoryMeta; selected: boolean; onToggle: () => void }) => {
  return (
    <button
...
const CategoryDropdown = ({ categories }: { categories: CategoryMeta[] }) => {
      ...
      >
        {categories.map((category) => (
          <CategoryItem
            key={category.slug}
            category={category}
            selected={current === category.slug}
            onToggle={() => onSelect(category.slug)}
          />
        ))}
      </div>
    </div>
