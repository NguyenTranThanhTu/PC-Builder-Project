...
// Static shopData đã được thay thế bằng dữ liệu từ Prisma truyền vào qua props
...
interface CategoryMeta { name: string; slug: string; productCount: number }
interface Props {
  products: Product[];
...
const ShopWithSidebar = ({ products, categories = [], pagination, view = "grid" }: Props) => {
  // view is driven by URL query param for consistency across navigations
  ...
  function renderPageNumbers(p: PaginationMeta) {
    const maxButtons = 7; // keep compact
    ...
    if (p.pageCount <= maxButtons) {
      ...
      const neighbors = [p.page - 1, p.page, p.page + 1].filter(
        (n) => n > first && n < last
      );
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          // Trigger a reload to server-render cleared filters
              ...
              <div className="rounded-lg bg-white shadow-1 pl-3 pr-2.5 py-2.5 mb-6">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex flex-wrap items-center gap-4">
                    ...
                    {pagination ? (
                      <p>
                        ...
                        of
                        <span className="text-dark"> {pagination.total} </span>
                        Products
                      </p>
                    ) : (
                      <p>
                  ...
                  <div className="flex items-center gap-2.5">
                    <Link
                      href={buildPageHref(pagination?.page ?? 1, pagination?.category, "grid")}
                    ...
                    <Link
                      href={buildPageHref(pagination?.page ?? 1, pagination?.category, "list")}
