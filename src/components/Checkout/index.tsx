...
const Checkout = () => {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const customerName = (firstName + " " + lastName).trim() || "Khách hàng";
    const payload = {
      customerName,
      ...
      items: cartItems.map((i: any) => ({ productId: i.productId as string, quantity: i.quantity })),
    };
  ...
  const shippingFee = items.length ? 15 : 0;
                  ...
                  <div className="pt-2.5 pb-8.5 px-4 sm:px-8.5">
                    ...
                    {items.map((it: any) => (
                      <div key={it.id} className="flex items-center justify-between py-5 border-b border-gray-3">
                        ...
                        <div>
                          <p className="text-dark text-right">${((it.discountedPrice || it.price) * it.quantity).toFixed(2)}</p>
                        </div>
                      </div>
                    ))}
