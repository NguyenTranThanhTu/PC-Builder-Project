...
const QuickViewModal = () => {
  ...
  const handleAddToCart = () => {
    if (!(product as any).productId) {
      // Fallback warning: productId cần cho checkout API; vẫn cho vào giỏ nhưng sẽ không đặt hàng được.
      console.warn("Thiếu productId thực trong QuickView item. Hãy thêm sản phẩm từ danh sách động (BestSeller/NewArrivals) để checkout.");
    }
                  ...
                  {product.imgs.thumbnails?.map((img, key) => (
                    ...
                    >
                      {img ? (
                        ...
                        <div className="w-[61px] h-[61px] bg-gray-3" aria-hidden="true" />
                      )}
                    </button>
                  ))}
                </div>
