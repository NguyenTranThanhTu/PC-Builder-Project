...
const ProductItem = ({ item }: { item: Product }) => {
  ...
  const handleAddToCart = () => {
    if (!item.productId) {
      console.warn(
        "Add to cart blocked: missing productId (DB id). Ensure products come from Prisma adapter."
      );
      return;
    }
    dispatch(
  ...
  const handleItemToWishList = () => {
    if (!item.productId) {
      console.warn(
        "Wishlist add blocked: missing productId (DB id). Ensure products come from Prisma adapter."
      );
      return;
    }
    dispatch(
  ...
  function resolveImage(): string {
    // Prefer adapter-provided single image
    const raw = (item as any).imageUrl || item.imgs?.previews?.[0] || item.imgs?.thumbnails?.[0];
    if (!raw) return "/images/products/product-1-sm-1.png";
    // Add leading slash if relative path without protocol
  ...
  return (
    <div className="group">
      ...
      >
        <Link href={item?.productSlug ? `/shop/${item.productSlug}` : "/shop-details"}> {item.title} </Link>
      </h3>
