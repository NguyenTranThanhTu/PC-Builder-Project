...
const SingleGridItem = ({ item }: { item: Product }) => {
  ...
  const handleAddToCart = () => {
    if (!item.productId) {
      // Guard: static mock item does not have real DB productId
      console.warn(
        "Add to cart blocked: missing productId (DB id). Replace static shopData with Prisma-backed products to enable checkout."
      );
      return;
    }
    dispatch(
  ...
  const handleItemToWishList = () => {
    if (!item.productId) {
      console.warn(
        "Wishlist add blocked: missing productId (DB id). Replace static shopData with Prisma-backed products."
      );
      return;
    }
    dispatch(
